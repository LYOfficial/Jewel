...
- [Jewel を選ぶ理由](#jewel-を選ぶ理由)
- [軽量設計とベンチマークデータ](#軽量設計とベンチマークデータ)
- [主な機能](#主な機能)
- [アーキテクチャ](#アーキテクチャ)
- [クイックスタート](#クイックスタート)
- [初回ログイン](#初回ログイン)
- [バックアップセンター](#バックアップセンター)
- [内蔵セルフアップデート](#内蔵セルフアップデート)
- [設定リファレンス](#設定リファレンス)
- [ローカル開発](#ローカル開発)
- [リポジトリ構成](#リポジトリ構成)
- [セキュリティ](#セキュリティ)
...
Jewel の軽量さは設計上の選択によるものです。素の Web UI はビルド成果物や別のフロントエンドサービスを必要とせず、サーバーは SQLite を使う単一の Node.js プロセスで動作します。デプロイ操作では、クラスター制御プレーンを追加せずホストの Docker API を直接利用します。

> [!NOTE]
> この節は公開ベンチマークの測定方法を定義するためのものです。検証されていない CPU、メモリ、ディスク使用量の数値は掲載しません。Ubuntu ホスト構成、アイドル時・負荷時のサンプル、ダッシュボードの記録を取得後に再現可能な実測値を追加します。

### ダッシュボードの測定範囲

ダッシュボードの **Jewel リソース** は Jewel コンテナ自身を対象にします。CPU とメモリは Docker 統計のスナップショット、ストレージは Jewel イメージ、書き込み層、`jewel-data` 名前付きボリュームの合計です。`/data` がホストのバインドマウントの場合、その内容は正確な合計として扱わず「未算入」と明示します。

### 追加予定のベンチマークデータ

| 項目 | 追加するデータ |
|---|---|
| テスト日と Jewel バージョン / Commit | 未記入 |
| Ubuntu と Docker のバージョン | 未記入 |
| CPU、メモリ、ディスク、仮想化の構成 | 未記入 |
| アイドル時の Jewel CPU / メモリ / ストレージ | 未記入 |
| デプロイまたは自動更新時のピーク | 未記入 |
| ダッシュボードの記録と採取コマンド | 未記入 |
