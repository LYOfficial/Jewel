...
在 **Debian 13.6 / KVM** 虛擬機（AMD EPYC 7302P 16 vCPU、15 GiB 記憶體、100 GB ext4 系統磁碟、Docker Engine 29.5.2）上的穩定狀態採樣如下：

| 指標 | Jewel | 同時刻主機 |
|---|---:|---:|
| CPU | **0.4%** | 4.8% |
| 記憶體 | **28.5 MiB** | 3.2 GiB / 15.6 GiB |
| 儲存 | **843.3 MiB** | 21.6 GiB / 98.2 GiB |

Jewel 閒置時約使用主機 **0.18%** 的記憶體。儲存由 501.1 MiB 映像、342.2 MiB 持久化資料與 0 B 可寫層組成；資料卷包含 SQLite、已複製專案與備份暫存，而非執行期行程負擔。映像已包含 Git、Docker CLI、Compose 與備份工具，可直接用於維運。
